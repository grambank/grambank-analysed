...
GB_PCA_df <- suppressMessages(read_tsv(file.path(OUTPUTDIR, 'ppca_language_values.tsv'))) %>%
...
for (p in GB_PCA_Families) {
    filename <- file.path(OUTPUTDIR, sprintf("pPCA_family_%s.tiff",  p$labels$title))
...
GB_PCA_multiGrid_family <- annotate_figure(
    ...
    bottom = text_grob("pPC1", size = 18, vjust = 0.4),
    left = text_grob("pPC2", size = 18, rot = 90, hjust = -1, vjust = 1)
)

filename <- file.path(OUTPUTDIR, "pPCA_multigrid_major_families.tiff")
filename_png <- file.path(OUTPUTDIR, "pPCA_multigrid_major_families.png")
cat(paste("writing", filename, "\n"))
...
filename <- file.path(OUTPUTDIR, "pPCA_all_families.tiff")
cat(paste("writing", filename, "\n"))
...
GB_PCA_multiGrid_macroareas <- annotate_figure(GB_PCA_multiGrid_macroareas,
    bottom = text_grob("pPC1", size = 18, vjust = 0),
    left = text_grob("pPC2", size = 18, rot = 90, hjust = -2)
)


filename <- file.path(OUTPUTDIR, "pPCA_multigrid_macroareas.tiff")
cat(paste("writing", filename, "\n"))
...
ggsave(file.path(OUTPUTDIR, "pPCA_family_Indo-European_highlights.tiff"), width = 7, height = 7)
ggsave(file.path(OUTPUTDIR, "pPCA_family_Indo-European_highlights.png"), width = 7, height = 7)
...
ggsave(file.path(OUTPUTDIR, "pPCA_family_Austroasiatic_highlights.tiff"), width = 7, height = 7)
ggsave(file.path(OUTPUTDIR, "pPCA_family_Austroasiatic_highlights.png"), width = 7, height = 7)